defmodule BaconNet.Modules.Iidx.Iidx31lobby do
  @moduledoc "Port of modules/iidx/iidx31lobby.py."

  alias BaconNet.{Core, E, State, XNode}

  @arena_host_key :iidx31lobby_arena_host
  @bpl_host_key :iidx31lobby_bpl_host
  ...
  def routes do
    %{
      ...
      tag: "lobby",
      handlers: [
        {"IIDX31lobby", "entry", :iidx31lobby_entry},
        {"IIDX31lobby", "update", :iidx31lobby_update},
        {"IIDX31lobby", "delete", :iidx31lobby_delete},
        {"IIDX31lobby", "bplbattle_entry", :iidx31lobby_bplbattle_entry},
        {"IIDX31lobby", "bplbattle_update", :iidx31lobby_bplbattle_update},
        {"IIDX31lobby", "bplbattle_delete", :iidx31lobby_bplbattle_delete}
      ]
    }
  end

  def iidx31lobby_entry(conn) do
  ...
  def iidx31lobby_update(conn) do
    {info, conn} = Core.process_request(conn)
    Core.send_response(conn, info, E.e("response", E.e("IIDX31lobby")))
  end

  def iidx31lobby_delete(conn) do
    {info, conn} = Core.process_request(conn)
  ...
  def iidx31lobby_bplbattle_entry(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
  ...
  def iidx31lobby_bplbattle_update(conn) do
    {info, conn} = Core.process_request(conn)
    Core.send_response(conn, info, E.e("response", E.e("IIDX31lobby")))
  end

  def iidx31lobby_bplbattle_delete(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
    address = XNode.child(node, "address")
    ga = address |> XNode.child("ga") |> text() |> String.split(~r/\s+/, trim: true)
