defmodule BaconNet.Modules.Iidx.Iidx30music do
  @moduledoc "Port of modules/iidx/iidx30music.py."
  ...
  def routes do
    %{
      ...
      tag: "local2",
      handlers: [
        {"IIDX30music", "getrank", :iidx30music_getrank},
        {"IIDX30music", "crate", :iidx30music_crate},
        {"IIDX30music", "reg", :iidx30music_reg},
        {"IIDX30music", "appoint", :iidx30music_appoint},
        {"IIDX30music", "arenaCPU", :iidx30music_arenacpu},
        {"IIDX30music", "retry", :iidx30music_retry},
        {"IIDX30music", "play", :iidx30music_play},
        {"IIDX30music", "nosave", :iidx30music_nosave},
        {"IIDX30music", "movieinfo", :iidx30music_movieinfo}
      ]
    }
  end

  def iidx30music_getrank(conn) do
    ...
    response =
      E.e(
        "response",
        E.e("IIDX30music", [E.e("style", type: play_style)] ++ m_nodes ++ top_nodes)
      )

    Core.send_response(conn, info, response)
  end

  def iidx30music_crate(conn) do
    {info, conn} = Core.process_request(conn)
    game_version = info.game_version
    ...
    response = E.e("response", E.e("IIDX30music", c_nodes))
    Core.send_response(conn, info, response)
  end

  def iidx30music_reg(conn) do
    {info, conn} = Core.process_request(conn)
    game_version = info.game_version
  ...
  def iidx30music_appoint(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
    ...
    response = E.e("response", E.e("IIDX30music", vals))
    Core.send_response(conn, info, response)
  end

  def iidx30music_arenacpu(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
              ...
              [E.e("index", bot_idx, __type: "s32")] ++
                Enum.map(py_range(music_count), fn music_idx ->
                  E.e("score_list", [
                    ...
                    E.e("ghost", cpu[music_idx][bot_idx]["ghost_data"], __type: "u8"),
  ...
  def iidx30music_retry(conn) do
    {info, conn} = Core.process_request(conn)

    response =
      E.e(
        "response",
        E.e("IIDX30music", [E.e("session", session_id: 1)], status: 0)
      )

    Core.send_response(conn, info, response)
  end

  def iidx30music_play(conn) do
    {info, conn} = Core.process_request(conn)

    response = E.e("response", E.e("IIDX30music"))

    Core.send_response(conn, info, response)
  end

  def iidx30music_nosave(conn) do
    {info, conn} = Core.process_request(conn)
  ...
  def iidx30music_movieinfo(conn) do
    {info, conn} = Core.process_request(conn)

    response =
      E.e(
        "response",
        E.e("IIDX30music", status: 0)
      )
