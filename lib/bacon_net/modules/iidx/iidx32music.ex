defmodule BaconNet.Modules.Iidx.Iidx32music do
  ...
  def routes do
    %{
      ...
      tag: "local",
      handlers: [
        {"IIDX32music", "getrank", :iidx32music_getrank},
        {"IIDX32music", "crate", :iidx32music_crate},
        {"IIDX32music", "reg", :iidx32music_reg},
        {"IIDX32music", "appoint", :iidx32music_appoint},
        {"IIDX32music", "arenaCPU", :iidx32music_arenacpu},
        {"IIDX32music", "retry", :iidx32music_retry},
        {"IIDX32music", "play", :iidx32music_play},
        {"IIDX32music", "nosave", :iidx32music_nosave},
        {"IIDX32music", "getranksub", :iidx32music_getranksub},
        {"IIDX32music", "movieinfo", :iidx32music_movieinfo}
      ]
    }
  end

  def iidx32music_getrank(conn) do
    ...
    response =
      E.e(
        "response",
        E.e("IIDX32music", [E.e("style", type: play_style)] ++ m_nodes ++ top_nodes)
      )

    Core.send_response(conn, info, response)
  end

  def iidx32music_crate(conn) do
    {info, conn} = Core.process_request(conn)
    game_version = info.game_version
    ...
    response = E.e("response", E.e("IIDX32music", c_nodes))
    Core.send_response(conn, info, response)
  end

  def iidx32music_reg(conn) do
    {info, conn} = Core.process_request(conn)
    game_version = info.game_version
  ...
  def iidx32music_appoint(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
    ...
    response = E.e("response", E.e("IIDX32music", vals))
    Core.send_response(conn, info, response)
  end

  def iidx32music_arenacpu(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
  ...
  def iidx32music_retry(conn) do
    {info, conn} = Core.process_request(conn)

    response =
      E.e(
        "response",
        E.e("IIDX32music", [E.e("session", session_id: 1)], status: 0)
      )

    Core.send_response(conn, info, response)
  end

  def iidx32music_play(conn) do
    {info, conn} = Core.process_request(conn)

    response = E.e("response", E.e("IIDX32music"))

    Core.send_response(conn, info, response)
  end

  def iidx32music_nosave(conn) do
    {info, conn} = Core.process_request(conn)
  ...
  def iidx32music_getranksub(conn) do
    {info, conn} = Core.process_request(conn)
  ...
  def iidx32music_movieinfo(conn) do
    {info, conn} = Core.process_request(conn)

    response =
      E.e(
        "response",
        E.e("IIDX32music", status: 0)
      )
