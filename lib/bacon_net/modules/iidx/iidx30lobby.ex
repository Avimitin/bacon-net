defmodule BaconNet.Modules.Iidx.Iidx30lobby do
  @moduledoc "Port of modules/iidx/iidx30lobby.py."

  alias BaconNet.{Core, E, State, XNode}

  @arena_host_key :iidx30lobby_arena_host
  @bpl_host_key :iidx30lobby_bpl_host
  ...
  def routes do
    %{
      ...
      tag: "lobby",
      handlers: [
        {"IIDX30lobby", "entry", :iidx30lobby_entry},
        {"IIDX30lobby", "update", :iidx30lobby_update},
        {"IIDX30lobby", "delete", :iidx30lobby_delete},
        {"IIDX30lobby", "bplbattle_entry", :iidx30lobby_bplbattle_entry},
        {"IIDX30lobby", "bplbattle_update", :iidx30lobby_bplbattle_update},
        {"IIDX30lobby", "bplbattle_delete", :iidx30lobby_bplbattle_delete}
      ]
    }
  end

  def iidx30lobby_entry(conn) do
  ...
  def iidx30lobby_update(conn) do
    {info, conn} = Core.process_request(conn)
    Core.send_response(conn, info, E.e("response", E.e("IIDX30lobby")))
  end

  def iidx30lobby_delete(conn) do
    {info, conn} = Core.process_request(conn)
  ...
  def iidx30lobby_bplbattle_entry(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
  ...
  def iidx30lobby_bplbattle_update(conn) do
    {info, conn} = Core.process_request(conn)
    Core.send_response(conn, info, E.e("response", E.e("IIDX30lobby")))
  end

  def iidx30lobby_bplbattle_delete(conn) do
    {info, conn} = Core.process_request(conn)
    node = Core.module_node(info)
    address = XNode.child(node, "address")
    ga = address |> XNode.child("ga") |> text() |> String.split(~r/\s+/, trim: true)
