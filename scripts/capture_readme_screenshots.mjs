...
try {
  ...
  async function screenshot(relativePath, cropToApp = true) {
    const clip = cropToApp
      ? await evaluate(`(() => {
        const app = document.querySelector('#root')?.firstElementChild;
        return {
          x: 0,
          y: 0,
          width: document.documentElement.clientWidth,
          height: Math.ceil(app?.getBoundingClientRect().bottom || window.innerHeight),
          scale: 1
        };
      })()`)
      : undefined;
  ...
  await navigate("/login", "Log in with your bacon-net account");
...
`);
  ...
  await screenshot("docs/webui-dashboard.png");

  await navigate("/scores?game=iidx&pageSize=25", "My scores");
  await waitFor(
  ...
  await navigate("/rankings?game=iidx&song=1207&chart=3&limit=10", "Rankings");
  await waitFor(
  ...
  await navigate("/settings/iidx_profile/1", "Profile settings");
  await waitFor(
  ...
  await navigate("/admin", "Permitted cabinet PCBIDs");
  await waitFor(
  ...
  await navigate("/", "Welcome back, neonkite");
  await waitFor(
    "document.body.innerText.includes('Game profiles')",
    "narrow dashboard",
  );
  await screenshot(
